...
- [106 — one branch of fourteen, and what that explains. [104](findings/104-one-pick-per-month.md) established exactly *what* `ical.js` does to `BYSETPOS` at `FREQ=YEARLY` — selects within each month instead of within the year — and said in so many words that the mechanism was not identified. **It is arm 8 of a fourteen-arm branch table.** `expand_year_days()` dispatches on the *set* of by-parts present (`partCount == 0`; `1: BYMONTH`; `1: BYMONTHDAY`; `2: BYMONTHDAY+BYMONTH`; `1: BYWEEKNO`; `2: BYWEEKNO+BYMONTHDAY`; `1: BYDAY`; **`2: BYDAY+BYMONTH`**; `2: BYDAY+BYMONTHDAY`; `3: BYDAY+BYMONTHDAY+BYMONTH`; `2: BYDAY+BYWEEKNO`; `3: BYDAY+BYWEEKNO+BYMONTHDAY`; `1: BYYEARDAY`; `2: BYYEARDAY+BYDAY`), and `check_set_position()` is called from **one** of them and nowhere else in the function — inside its `for (let month of this.by_data.BYMONTH)` loop, against a `by_month_day` array rebuilt per month and never accumulated across the year. **The naming explains more than 104 asked about.** 104's four separately-reasoned controls collapse into one fact about where a call appears: single-valued `BYMONTH` is correct because it takes the *same* arm and per-month coincides with per-year; no `BYMONTH` drops `BYSETPOS` because arm 7 never consults it; `BYMONTHDAY` instead of `BYDAY` drops it because arm 4 never consults it; `FREQ=MONTHLY` is a different function. **It also corrects [074](findings/074-what-reproducing-an-output-attributes.md)'s defect E**, stated as *"`BYSETPOS` dropped unless the day set came from `BYDAY`"* — too generous, because arm 7 is `BYDAY` alone and drops it too, as do arms 9, 10, 11 and 14; the surviving case is `BYDAY` **with** `BYMONTH` **and nothing else**. That widens E rather than narrowing it, so no id is unclaimed and [102](findings/102-the-residual-had-no-producer.md)'s producer still re-derives **23 of 23, residual 0**. **And it answers, with a no, the question [105](findings/105-the-month-that-rolled-over.md) closed on:** `next_year()` has no counterpart to 105's month-rollover asymmetry and cannot grow one, because `next_month()` carries **two inline copies** of the set-position test that fell out of step while `next_year()` carries none — on entry and on rollover alike it calls this one function, whose single `BYSETPOS` site tests `spIndex + 1` **and** `spIndex - by_month_day.length` in the same loop. Measured as well as read: the negative spellings `BYSETPOS=-1` and `-5` behave identically to the two positive probes 104 published. **105's defect is a consequence of duplicated logic, not of the year/month boundary — and my operating note's prediction that 103 and 105 were two faces of one habit in this file was wrong.** Two further structural observations fall out. **Two of the three `// TODO unimplemented` arms can never execute**: arms 6 and 12 both pair `BYWEEKNO` with `BYMONTHDAY`, which `init()` refuses first with `throw new Error("BYWEEKNO does not fit to BYMONTHDAY")` — a message that presents itself as rule validation while RFC 5545 §3.3.10 marks both parts **Expand** at `FREQ=YEARLY` and states no prohibition on the pair. Corpus case `7b92dd911bbb` lands on it, and `dateutil`, `rrule.js` and dmfs `lib-recur` all return the same 19 occurrences where `ical.js` refuses. **That is recorded, not claimed:** three implementations agreeing is suggestive, the RFC's silence is not a licence, and a refusal matching `libical`'s own behaviour deserves its own controls, which it does not get here. **And an unimplemented arm is indistinguishable from a rule with no occurrences**: arm 5, `BYWEEKNO` alone, produces no error at all, because it yields no days, so `init()`'s year search runs to its `untilYear` default of 20000 — **17,974 calls to `expand_year_days()`**, counted by the reproducer rather than quoted into it — and raises `InvalidRecurrenceRuleError`, which `ical.js` deliberately swallows into an empty iterator, while the `BYWEEKNO`+`BYMONTHDAY` pair raises a plain `Error` that propagates. That unbounded search is [103](findings/103-the-year-the-iterator-gave-up.md)'s already-published willing-to-wait half, **cited and not re-claimed — a published defect reached from a new direction is not a new defect.** The reproducer walks every arm with a rule that lands on it and fails if any behaves differently from the code reading; all fifteen rows match, and `dropped` is decided by comparing `ical.js` against **`ical.js` with `BYSETPOS` removed** rather than against the reference, so a row can be called dropped without first settling what the right answer is. **Not claimed:** no score moves, no case changes bucket, `cases_id` unchanged, `RESULTS.md` untouched, 074's residual stays 0 with no id claimed or released, nothing filed upstream (rule 27), the arm list is this vendored copy of `ical.js` 2.2.1 and not upstream `main`, and `_byDayAndMonthDay()` is still untested. Two incidental oddities are **written down and deliberately left unexamined** rather than folded in — arm 11 tests `BYWEEKNO.indexOf(weekno)` with no `>= 0`, and the `BYMONTH`+`BYWEEKNO` pre-pass indexes its `validWeeks` map by a position into `BYWEEKNO` instead of by a week number. New standing **rule 111: when a finding has established a predictor but not a mechanism, the mechanism is worth going back for, because a mechanism explains the controls as well as the claim** — 104 needed four separately-reasoned controls to pin its predictor, and one call site explains all four, corrects an older finding's scope, and answers a question a later finding left open. A predictor tells you what will happen; a mechanism tells you what else is true](findings/106-one-branch-of-fourteen.md).
